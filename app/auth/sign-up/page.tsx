...
export default function SignUpPage() {
  const { signUpWithEmailPassword, signInWithMagicLink } = useAuth();
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setLoading(false);
    if ((res as any)?.error) {
      setError((res as any).error.message || "Error al crear la cuenta");
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950 p-6">
      <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-800">
        ...
        <p className="mt-6 text-sm text-gray-600 dark:text-gray-300">
          ¿Ya tienes cuenta? {" "}
          <Link href="/auth/sign-in" className="text-blue-600 hover:underline">Inicia sesión</Link>
        </p>
      </div>
    </div>
  );
